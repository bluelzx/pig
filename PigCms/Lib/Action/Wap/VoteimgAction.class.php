...
class VoteimgAction extends WapAction
{
	...
	public function index()
	{
		...
		if (S($this->action_id . '_' . $this->token . '_old_update') == '') {
			if (M('voteimg_menus')->where(array(
	...
	'menu_name' => array('like', '活动日期%至%')
				...
				S($this->action_id . '_' . $this->token . '_old_update', 1);
			}
		}
	...
	{
		$cache_voter = F($this->action_id . '_' . $this->token . '_' . $this->wecha_id);

		if ($cache_voter['user_id'] != '') {
			$voter = $cache_voter;
		}
		else {
			$voter = M('voteimg_users')->where(array('vote_id' => $this->action_id, 'token' => $this->token, 'wecha_id' => $this->wecha_id))->find();
			F($this->action_id . '_' . $this->token . '_' . $this->wecha_id, array('user_id' => $voter['user_id'], 'wecha_id' => $voter['wecha_id']));
		}
	...
	private function clear_vote_day()
	{
		...
		if (S($this->token . '_' . $this->action_id . '_vote_day') == '') {
			$where = 'vote_id = ' . $this->action_id . ' and token = \'' . $this->token . '\' and vote_time < \'' . $today_time . '\'';

			if (M('voteimg_users')->where($where)->find()) {
				M('voteimg_users')->where($where)->save(array('votenum_day' => 0, 'vote_today' => 0));
				S($this->token . '_' . $this->action_id . '_vote_day', 1, $cache_time);
			}
		}
	}
}
