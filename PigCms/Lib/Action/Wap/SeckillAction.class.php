...
class SeckillAction extends WapAction
{
	...
	public function index()
	{
		...
		if ($this->wecha_id) {
			$res = $this->check_user(array('user_wechaid' => $this->wecha_id));
			...
			if ($res) {
				...
				$updata = M('seckill_users')->where(array('user_wechaid' => $this->wecha_id))->save($data);
			}
			else {
		...
		$min = M('seckill_users')->where(array('user_aid' => $this->action_id))->field('user_mintime')->find();
		...
		$scode1 = M('seckill_users')->where(array('user_id' => $_SESSION['suid']))->getField('user_shareid');
		...
		$udata = M('seckill_users')->where(array('user_aid' => $this->action_id, 'user_shareid' => $scode1))->order('user_mintime desc')->count();
		$this->assign('share', $share_data);
		$this->assign('num', $udata);
		$this->display();
	}
	...
	public function shop_invite()
	{
		...
		if ($this->share_code && $resultu && ($resultu['user_wechaid'] != $this->wecha_id) && $this->wecha_id) {
			$bool_share = M('seckill_share')->where(array('user_share' => $this->share_code, 'share_wechaid' => $this->wecha_id))->find();

			if (!$bool_share) {
				$mintime = rand($this->action['rand_min_time'], $this->action['rand_max_time']);
				$where = array('user_shareid' => $this->share_code);
	...
	public function pay()
	{
		if (IS_POST) {
			...
			$url = U('Alipay/pay', array('token' => $token, 'wecha_id' => $wecha_id, 'price' => $price, 'from' => 'Seckill', 'orderName' => $orderName, 'orderid' => $orderid));
			header('Location:' . $url);
		}
		else {
