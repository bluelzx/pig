...
class NumqueueAction extends WapAction
{
	...
	public function admin_login()
	{
		if (IS_POST) {
			...
			$action_id = $this->_post('action_id', 'intval');
			$token = $this->_post('token', 'trim');
			$post_pwd = $this->_post('password', 'trim');
			$password = M('numqueue_store')->where(array('id' => $store_id))->getField('password');

			if ($post_pwd == $password) {
				$exists = M('numqueue_admin')->where(array('token' => $token, 'store_id' => $store_id))->find();

				if ($exists) {
					M('numqueue_admin')->where(array('token' => $token, 'store_id' => $store_id))->save(array('password' => $password, 'wecha_id' => $this->wecha_id));
				}
				else {
					$data = array('password' => $password, 'token' => $token, 'wecha_id' => $this->wecha_id, 'store_id' => $store_id);
					M('numqueue_admin')->add($data);
				}

				header('Location:/index.php?g=Wap&m=Numqueue&a=admin_manage&store_id=' . $store_id . '&token=' . $token . '&id=' . $action_id);
				exit();
			}
			else {
