		...
		if (ALI_FUWU_GROUP) {
			...
			foreach ($fuwuuserlist as $fuwuvo) {
				...
				if ($fuwuuserinfo == '') {
					...
					$id_userinfo = M('userinfo')->add(array('token' => $this->token, 'wecha_id' => $fuwuvo['wecha_id'], 'issub' => 1, 'portrait' => $fuwuvo['avatar'], 'truename' => $fuwuvo['real_name'], 'nickname' => $fuwuvo['real_name'], 'wechaname' => $fuwuvo['real_name'], 'sex' => $fuwuvo['gender']));
				}
			}
		}
	}
