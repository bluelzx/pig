...
class HotelsAction extends UserAction 
{
	...
	public function orders() 
	{
		...
		$where = "`token`='" . $this->_session('token') . "' AND `cid`='{$this->_cid}'";//array('token' => $this->_session('token'), 'cid' => $this->_cid);
		if ($status) {
			$where .= " AND `startdate`<='{$status}' AND `enddate`>'{$status}'";
// 			$where['startdate'] = array('ELT', $status);
// 			$where['enddate'] = array('GT', $status);
		}
		if ($searchkey) {
			$where .= " AND (`name` like '%{$searchkey}%' OR `tel` like '%{$searchkey}%' OR `orderid` like '%{$searchkey}%')";
		}
