...
class CarAction extends UserAction{
    ...
    public function default_cat(){
        $info = array();
        $info[0]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=showcar&token='.$this->token.'&wecha_id={wechat_id}';
        $info[0]['img'] = '/tpl/static/attachment/icon/white/6.png';
        $info[0]['name'] = '车型欣赏';

        $info[1]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=aboutus&token='.$this->token.'&wecha_id={wechat_id}';
        $info[1]['img'] = '/tpl/static/attachment/icon/white/5.png';
        $info[1]['name'] = '关于我们';
         
        $info[2]['url']  = '{siteUrl}/index.php?g=Wap&m=Panorama&a=index&token='.$this->token.'&wecha_id={wechat_id}&sgssz=mp.weixin.qq.com';
        $info[2]['img'] = '/tpl/static/attachment/icon/white/13.png';
        $info[2]['name'] = '全景看车';
        
        $info[3]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=tool&token='.$this->token.'&wecha_id={wechat_id}';
        $info[3]['img'] = '/tpl/static/attachment/icon/white/2.png';
        $info[3]['name'] = '实用工具';
        
        $info[4]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=brands&token='.$this->token.'&wecha_id={wechat_id}';
        $info[4]['img'] = '/tpl/static/attachment/icon/white/1.png';
        $info[4]['name'] = '经销车型';

        $info[5]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=salers&token='.$this->token.'&wecha_id={wechat_id}';
        $info[5]['img'] = '/tpl/static/attachment/icon/white/7.png';
        $info[5]['name'] = '销售顾问';
         
        $info[6]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=CarReserveBook&token='.$this->token.'&wecha_id={wechat_id}';
        $info[6]['img'] = '/tpl/static/attachment/icon/white/14.png';
        $info[6]['name'] = '在线预约';
        
        $info[7]['url']  = '{siteUrl}/index.php?g=Wap&m=Car&a=owner&token='.$this->token.'&wecha_id={wechat_id}';
        $info[7]['img'] = '/tpl/static/attachment/icon/white/11.png';
        $info[7]['name'] = '车主关怀';
    ...
    public function cat_add(){
        ...
        $info   = M('Carcat')->where($where)->find();
        
        if (IS_POST) {
            if(D('Carcat')->create()){
                if($info){
                    M('Carcat')->where(array('token'=>$this->token,'id'=>$this->_post('id')))->save($_POST);
                    $this->success('修改成功',U('Car/cat',array('token'=>$this->token,'type'=>$type)));
                }else{
                    $_POST['token'] = $this->token;
                    M('Carcat')->add($_POST);
                    $this->success('添加成功',U('Car/cat',array('token'=>$this->token,'type'=>$type)));
                }
                
            }else{
                $this->error(D('Carcat')->getError());
            } 
        }else{
    ...
    public function cat_del(){
        $type   = $this->_get('type','trim');
        $id     = $this->_get('id','intval');
        if(M('Carcat')->where(array('token'=>$this->token,'id'=>$id,'system'=>'0'))->delete()){
            $this->success('删除成功',U('Car/cat',array('token'=>$this->token,'type'=>$type)));
        }
    }
}
