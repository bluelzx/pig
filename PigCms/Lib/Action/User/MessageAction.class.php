		...
		if (IS_POST) {
			...
			if ($access_token) {
				...
				foreach ($imgs as $img) {
					...
					if ($rt['rt']) {
						$str .= $comma . '{"thumb_media_id":"' . $rt['media_id'] . '","author":"","title":"' . $img['title'] . '","content_source_url":"' . $content_source_url . '","content":"' . str_replace(array('"', '"/upload'), array('\\"', '"' . C('site_url') . '/upload'), html_entity_decode($img['info'])) . '","digest":"' . $img['text'] . '","show_cover_pic":' . $img['showpic'] . '}';
						$comma = ',';
					}
					else {
