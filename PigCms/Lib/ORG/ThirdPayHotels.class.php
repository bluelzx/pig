	...
	public function index($orderid, $paytype = '', $third_id = ''){
		...
		if ($order = M('Hotels_order')->where(array('orderid' => $orderid))->find()) {
			...
			if ($order['paid']) {
				...
				Sms::sendSms($order['token'] . "_" . $order['cid'], "顾客{$order['name']}刚刚对订单号：{$orderid}的订单进行了支付，请您注意查看并处理",$company['mp']);//自行增加短信接收号，不增加将默认发
				$model = new templateNews();
				$model->sendTempMsg('OPENTM202521011', array('href' => U('Hotels/my',array('token' => $order['token'], 'wecha_id' => $order['wecha_id']), true, false, true), 'wecha_id' => $order['wecha_id'], 'first' => '预订房间提醒', 'keyword1' => $orderid, 'keyword2' => date("Y年m月d日H时i分s秒"), 'remark' => '预订房间成功，感谢您的光临，欢迎下次再次光临！'));
