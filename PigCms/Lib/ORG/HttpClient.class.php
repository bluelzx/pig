...
class HttpClient {
	...
	protected $config = array(
		...
		CURLOPT_USERAGENT      => 'pigcms-lixiang/v1.0',
	...
	private function _request($url, $params) {
		...
		if ('post' == $this->method) {
			...
			curl_setopt($this->ch, CURLOPT_POSTFIELDS, is_array($params['post']) ? http_build_query($params['post']) : $params['post']);
		}
		if ($params['header']) {
			curl_setopt($this->ch, CURLOPT_HTTPHEADER, $this->setHeader($params['header'])); 
		}
